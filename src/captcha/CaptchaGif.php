...
class CaptchaGif
{
    ...
    private static $FontFilePath = __DIR__ . '/../image/font/font/'; //相对地本代码文件的位置
    private static $FontFileName = ['Tahoma.TTF'];// array("1.ttf", "2.ttf", "3.ttf", "4.ttf", "5.ttf", "6.ttf", "7.ttf", "8.ttf"); //
    ...
    public static function Draw($L = 4, $F = 1, $W = 150, $H = 30, $MixCnt = 2, $lineGap = 0, $noisyCnt = 10, $sessionName = "Code")
    {
        ...
        for ($i = 0; $i < $L; $i++) {
            ...
            self::$Code .= $rCode[] =  substr(self::$Chars, mt_rand(0, strlen(self::$Chars) - 1), 1);
        }
        
        if (!is_file(self::$FontFilePath . self::$FontFileName[0])) {
        ...
        // 随机背景颜色
        $bgRGB = [mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255)];
        ...
        for ($i = 0; $i < $F; $i++) {
            ...
            for ($k = 0; $k < $MixCnt; $k++) {
                ...
                imagearc($img, $W - floor($wr / 2), floor($hr / 2), $wr, $hr, mt_rand(90, 180), mt_rand(180, 270), $lineColor);
                unset($lineColor);
                unset($wr, $hr);
            }
            ...
            if ($i != 0 || $F <= 1) {
                //文字
                $foreColor = imagecolorallocate($img, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));
                for ($j = 0; $j < $L; $j++) {
                    $fontFile = self::$FontFilePath . self::$FontFileName[mt_rand(0, count(self::$FontFileName) - 1)];
                    if (!file_exists($fontFile)) {
                        imagestring($img, 4, self::$TextMargin + $j * self::$TextGap, ($H - mt_rand($H / 2, $H)), self::$Code[$j], $foreColor);
                    } else {
                        imageTTFtext($img, mt_rand(15, 18), mt_rand(-15, 15), self::$TextMargin + $j * self::$TextGap, ($H - mt_rand(7, 10)), $foreColor, $fontFile, self::$Code[$j]);
                    }    
                }
                unset($foreColor);
            }
        ...
        echo self::CreateGif($Imdata, 50); // 100 = 1秒?
        unset($Imdata);
    }
    
    /**
     * 合成GIF
     * @param array $GIF_src 图片集合
     * @param int $GIF_dly 间隔
     * @param number $GIF_lop
     * @param number $GIF_dis
     * @param number $GIF_red
     * @param number $GIF_grn
     * @param number $GIF_blu
     * @param string $GIF_mod 图片模式, 可能的值:
     * - 'url' : 图片URL集合
     * - 'bin' : 图片二进制集合
     * @return string|number
     */
    private static function CreateGif(array $GIF_src, $GIF_dly = 10, $GIF_lop = 0, $GIF_dis = 0, $GIF_red = 0, $GIF_grn = 0, $GIF_blu = 0, $GIF_mod = 'bin') {
        if (!is_array($GIF_src)) { // && !is_array($GIF_src)
